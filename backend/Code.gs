 ...
 *  4. Helpers    - tanggal, predikat, halaman/surah terakhir, relasi penyimak-santri
 ...
 */
/**
 * Menentukan "halaman terakhir" (dari jenis 'Setoran Metode Ummi', untuk level
 * Jilid) DAN "surah terakhir" (dari jenis 'Hafalan Baru', untuk yang sudah
 * masuk Al-Qur'an) seorang santri secara TERPISAH -- keduanya bisa saja ada
 * bersamaan kalau siswa pernah dapat catatan di kedua jenis tsb. Murojaah &
 * Tilawah tidak dihitung karena itu pengulangan materi lama, bukan capaian baru.
 * @param {Array} rowsSantri baris-baris SHEET_SETORAN milik SATU santri (sudah difilter)
 * @return {Object} { halaman: {label,tanggal}|null, surah: {label,tanggal}|null }
 */
function computeCapaianTerakhir(rowsSantri) {
  const ummiRows = rowsSantri.filter(r => r.jenis === 'Setoran Metode Ummi')
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));
  const hafalanRows = rowsSantri.filter(r => r.jenis === 'Hafalan Baru')
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));
  ...
  const surah = hafalanRows.length ? {
    label: (hafalanRows[0].surah_selesai || hafalanRows[0].surah || '-') + ' : Ayat ' + (hafalanRows[0].ayat_selesai || hafalanRows[0].ayat_mulai || '-'),
    tanggal: hafalanRows[0].tanggal
  } : null;

  return { halaman: halaman, surah: surah };
}
...
function actionGetSantri(token, payload) {
  ...
  if (payload.with_posisi) {
    ...
    data = data.map(s => {
      ...
      const capaian = computeCapaianTerakhir(rows);
...
function actionGetStatistik(token, payload) {
  ...
  const peringkat = santriScope.map(santri => {
    ...
    // Halaman/surah terakhir dihitung dari SELURUH riwayat (bukan cuma periode
    // filter statistik ini), supaya benar-benar mencerminkan capaian terkini siswa.
    const rowsAllTime = setoranAllTime.filter(s => String(s.santri_id) === String(santri.id));
    const capaian = computeCapaianTerakhir(rowsAllTime);
    return {
...
function actionGetRekap(token, payload) {
  ...
  const hasil = santriList.map(santri => {
    ...
    // Halaman/surah terakhir DI AKHIR PERIODE INI (bukan all-time) -- supaya
    // rapor lama yang dicetak ulang tetap akurat menampilkan capaian saat itu,
...
function actionGetProgressSiswa(token, payload) {
  ...
  // ---------- Halaman & surah terakhir (all-time, tidak dibatasi rentang filter) ----------
