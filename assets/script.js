...
const TF = (() => {
  ...
  async function render() {
    ...
    // Halaman berbasis tabel pakai skeleton shimmer (menyerupai baris tabel
    // yang akan muncul); halaman lain pakai spinner + pesan bergantian.
    const SKELETON_VIEWS = ['setoran', 'santri', 'kelas', 'statistik', 'presensi', 'users'];
    content.innerHTML = SKELETON_VIEWS.includes(state.view)
      ? skeletonTableHtml(7)
      : loadingHtml();
  ...
  async function submitSetoran() { // mutasi: invalidate setoran + dashboard
    ...
    contentEl.innerHTML = skeletonTableHtml(5);
  ...
  async function loadRiwayatPresensi() {
    ...
    if (!el) return;
    el.innerHTML = skeletonTableHtml(4);
    const payload = {};
  ...
  // ============================================================
  // LOADING STATES — dua gaya, dipakai sesuai konteks:
  //  - loadingHtml(): spinner + logo berdenyut + pesan bergantian
  //    (untuk halaman non-tabel: dashboard, progress, rekap, dll)
  //  - skeletonTableHtml(): kotak abu-abu shimmer menyerupai baris
  //    tabel (untuk halaman berbasis tabel: setoran, santri, dll)
  // ============================================================
  ...
  function skeletonTableHtml(rows) {
    const colWidths = [4, 20, 14, 12, 14, 12, 10]; // proporsi lebar kolom, cukup umum utk berbagai tabel
    const rowHtml = colWidths.map(w => `<div class="tf-skel-bar" style="width:${w}%;"></div>`).join('');
    return `<div class="tf-skel-wrap">${Array.from({ length: rows || 6 }).map(() => `<div class="tf-skel-row">${rowHtml}</div>`).join('')}</div>`;
  }
  // ============================================================
