...
const TF = (() => {
  ...
  function openSetoranModal() {
    const santriOptions = (state.cache.santriSetoran || state.cache.santri || [])
      .map(s => `<option value="${s.id}" data-level="${escapeHtml(s.level_ummi || '')}" data-posisi="${escapeHtml(s.posisi_terakhir || '-')}">${escapeHtml(s.nama)}</option>`).join('');
    openModal(`
  ...
  function renderSetoranFields() {
    ...
    const posisi = sel.options.length ? sel.options[sel.selectedIndex].dataset.posisi : '-';
    ...
    if (posisiEl) {
      posisiEl.innerHTML = (posisi && posisi !== '-')
        ? `<div class="tf-empty" style="text-align:left;margin-bottom:12px;">📍 Posisi terakhir: <b>${escapeHtml(posisi)}</b></div>`
  ...
  function renderPeringkat() {
    ...
    const rows = data.map((d, i) => `
      <tr>
        ...
        <td>${escapeHtml(d.posisi_terakhir||'-')}</td>
    ...
    el.innerHTML = `
      <div class="tf-table-wrap">
        <table class="tf-table">
          <thead><tr><th>#</th><th>Nama</th><th>Kelas</th><th>Level Ummi</th><th>Posisi Terakhir</th><th>Total Setoran</th><th>Rata-rata Nilai</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
    `;
  ...
  async function loadProgress() {
    ...
    el.innerHTML = `
      ...
      <div class="tf-empty" style="text-align:left;margin-bottom:14px;">📍 Posisi terakhir: <b>${escapeHtml(res.posisi_terakhir ? res.posisi_terakhir.label : 'Belum ada riwayat')}</b>${res.posisi_terakhir ? ` <span style="color:#64748b;">(${escapeHtml(res.posisi_terakhir.tanggal ? String(res.posisi_terakhir.tanggal).substring(0,10) : '-')})</span>` : ''}</div>
      <div class="tf-stats">
        <div class="tf-stat-card c-cyan"><div class="label">TOTAL SETORAN</div><div class="value">${res.total_setoran}</div></div>
      ...
      <div class="tf-rapor-a4">
        ...
        <div class="rp-info-box">
          <div class="rp-info-grid">
            ...
            <div class="row"><span class="lbl">Posisi Terakhir</span><span class="col">:</span><span class="val">${escapeHtml(d.posisi_terakhir||'-')}</span></div>
    ...
    if (view === 'santri') {
      ...
      return `<table class="tf-table"><thead><tr>
        ...
        ${data.map((s, i) => `<tr>
          ...
          <td>${escapeHtml(s.level_ummi||'-')}</td>
          <td>${escapeHtml(s.posisi_terakhir||'-')}</td>
